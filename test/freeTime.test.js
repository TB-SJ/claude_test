...
// tzOffset 0 → UTC is local; workday defaults to 06:00–21:00 (awake window).
const RULES = { tzOffsetMinutes: 0 };
// "now" before the work window so the full day is considered (deterministic).
const BEFORE = { referenceDate: new Date('2026-08-05T05:00:00Z') };
...
test('freeForDays returns the gaps around events within work hours', () => {
  ...
  assert.deepEqual(
    ...
    [
      ['2026-08-05T06:00:00.000Z', '2026-08-05T10:00:00.000Z', 240],
      ['2026-08-05T11:00:00.000Z', '2026-08-05T13:00:00.000Z', 120],
      ['2026-08-05T14:00:00.000Z', '2026-08-05T21:00:00.000Z', 420],
    ]
  );
});

test('a fully open day is one 15-hour slot; overlapping events merge', () => {
  const [empty] = freeForDays([], ['2026-08-05'], RULES, BEFORE);
  assert.equal(empty.slots.length, 1);
  assert.equal(empty.slots[0].minutes, 900); // 06:00–21:00
  ...
  // One merged busy block 10–13 → free 06–10 and 13–21.
  assert.deepEqual(day.slots.map((s) => s.minutes), [240, 480]);
});

test('free time for today starts at "now", not the start of the workday', () => {
  // "now" = 12:30; a meeting 14:00–15:00. Remaining free = 12:30–14:00 and 15:00–21:00.
  ...
  assert.deepEqual(
    ...
    [
      ...
      ['2026-08-05T15:00:00.000Z', '2026-08-05T21:00:00.000Z'],
    ]
  );
});

test('a day whose work window has fully passed shows no free time', () => {
  const after = { referenceDate: new Date('2026-08-05T22:00:00Z') }; // past 21:00
...
test('coveringWindow spans the work hours of all days', () => {
  ...
  assert.equal(win.start, '2026-08-03T06:00:00.000Z');
  assert.equal(win.end, '2026-08-07T21:00:00.000Z');
});
